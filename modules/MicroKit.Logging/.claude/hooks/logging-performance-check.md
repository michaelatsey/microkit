# logging-performance-check Hook

Automatically flags potential performance regressions when hot-path files are modified. Triggers the `logging-performance-reviewer` agent via a PostToolUse hook.
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .claude/hooks/scripts/logging-performance-check.sh \"$CLAUDE_TOOL_INPUT_PATH\""
          }
        ]
      }
    ]
...
**Location:** `.claude/hooks/scripts/logging-performance-check.sh`
...
if [ $VIOLATIONS -gt 0 ]; then
  ...
  echo "   Run /logging-review-performance --file $CHANGED_FILE for full analysis."
...
- For blocking enforcement, run `/logging-review-performance` explicitly
