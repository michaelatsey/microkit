# logging-architecture-check Hook

Automatically invokes the `logging-architect` agent when project references or `.csproj` files change.
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .claude/hooks/scripts/logging-architecture-check.sh \"$CLAUDE_TOOL_INPUT_PATH\""
          }
        ]
      }
    ]
...
**Location:** `.claude/hooks/scripts/logging-architecture-check.sh`
