# logging-dependency-check Hook

Invokes the `logging-dependency-guardian` agent automatically when any `<PackageReference>` or `<ProjectReference>` is added or changed.
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .claude/hooks/scripts/logging-dependency-check.sh \"$CLAUDE_TOOL_INPUT_PATH\""
          }
        ]
      }
    ]
...
**Location:** `.claude/hooks/scripts/logging-dependency-check.sh`
...
This hook handles **fast deterministic checks** via shell. For deeper graph analysis (transitive dependencies, NuGet resolution), invoke `/logging-review-architecture` which runs the `logging-dependency-guardian` agent.
