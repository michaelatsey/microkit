    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .claude/hooks/scripts/analyzer-check.sh \"$CLAUDE_TOOL_INPUT_PATH\""
          }
        ]
      }
    ]
...
**Location:** `.claude/hooks/scripts/analyzer-check.sh`
...
if [ $VIOLATIONS -gt 0 ]; then
  echo "⚠️  $VIOLATIONS warning(s). Run /review-architecture or Use agent analyzer-reviewer."
fi
