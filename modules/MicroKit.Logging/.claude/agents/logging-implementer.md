...
2. `.claude/rules/logging-architecture.md` — layer boundaries
3. `.claude/rules/logging-naming.md` — naming conventions
4. `.claude/rules/logging-performance.md` — performance constraints
5. The relevant rule file for the target component:
   - Enricher → `.claude/rules/logging-abstractions.md`
   - Provider → `.claude/rules/logging-dependencies.md`
   - Analyzer → `.claude/rules/logging-analyzers.md`
   - Generator → `.claude/rules/logging-generators.md`
   - OTEL work → `.claude/rules/logging-opentelemetry.md`
   - Diagnostics → `.claude/rules/logging-diagnostics.md`
...
- Hot path: yes/no
- Allocation budget: X bytes/op (from `.claude-context/standards/logging-performance-budget.md`)
...
| Agent | When |
|-------|------|
| `logging-dependency-guardian` | After any `.csproj` change |
| `logging-api-reviewer` | If Abstractions surface changes |
| `logging-performance-reviewer` | If hot-path code added |
| `logging-observability-reviewer` | If ActivitySource or OTEL touched |
