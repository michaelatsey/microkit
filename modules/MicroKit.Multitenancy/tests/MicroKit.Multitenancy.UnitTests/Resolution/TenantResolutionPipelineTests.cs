...
namespace MicroKit.Multitenancy.UnitTests.Resolution;
...
public sealed class TenantResolutionPipelineTests
{
    ...
    private static Multitenancy.TenantId NewId() => Multitenancy.TenantId.NewId();
    ...
    private static ITenantResolutionStrategy FailingStrategy(int order)
    {
        ...
        s.TryResolveAsync(Arg.Any<CancellationToken>())
            .Returns(ValueTask.FromResult(
                Failure<Multitenancy.TenantId>(MultitenancyErrors.ResolutionFailed)));
        return s;
    }

    private static ITenantResolutionStrategy SucceedingStrategy(int order, Multitenancy.TenantId id)
    {
        var s = Substitute.For<ITenantResolutionStrategy>();
        s.Order.Returns(order);
        s.TryResolveAsync(Arg.Any<CancellationToken>())
    ...
    public async Task ResolveAsync_ExecutesStrategiesInAscendingOrder()
    {
        ...
        strategy20.TryResolveAsync(Arg.Any<CancellationToken>())
            ...
            {
                callOrder.Add(20);
                return ValueTask.FromResult(Failure<Multitenancy.TenantId>(MultitenancyErrors.ResolutionFailed));
            });
