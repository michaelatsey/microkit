...
1. `src/MicroKit.Multitenancy.AspNetCore/Strategies/{Name}TenantResolutionStrategy.cs`
   - `sealed class` implementing `ITenantResolutionStrategy`
   - `Order` property
   - `TryResolveAsync` returning `Result<TenantId>` (never throws)
   - XML documentation

2. Test: `tests/MicroKit.Multitenancy.UnitTests/Strategies/{Name}TenantResolutionStrategyTests.cs`
   - Success case
