...
{
    // Anchor: TenantId lives only in Abstractions (not in Core, which shares the MicroKit.Tenancy namespace).
    ...
    public void Abstractions_DoesNotReferenceMultitenancyCore()
    {
        ...
        var refs = AbstractionsAssembly.GetReferencedAssemblies();
        refs.Any(a => a.Name == "MicroKit.Tenancy").ShouldBeFalse();
    }
    ...
    public void Abstractions_ContainsNoConcreteContextAccessorImplementation()
    {
        ...
        // Abstractions defines the contract; the implementation is in MicroKit.Tenancy (Core).
