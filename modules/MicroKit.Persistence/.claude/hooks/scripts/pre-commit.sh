...
# Only run if MicroKit.Persistence files are staged
if ! echo "$CHANGED_FILES" | grep -q "modules/MicroKit.Persistence/"; then
  exit 0
fi

echo "🔍 MicroKit.Persistence pre-commit checks..."
...
# 1. Build
echo "→ Building..."
dotnet build "$MODULE_ROOT/MicroKit.Persistence.slnx" -c Debug --nologo -q
if [ $? -ne 0 ]; then
  echo "❌ Build failed. Commit blocked."
  exit 1
fi

# 2. Unit tests only (fast — no DB required)
echo "→ Running unit tests..."
dotnet test "$MODULE_ROOT/tests/MicroKit.Persistence.UnitTests/" \
  --no-build --nologo -q \
...
dotnet test "$MODULE_ROOT/tests/MicroKit.Persistence.ArchitectureTests/" \
  --no-build --nologo -q
if [ $? -ne 0 ]; then
